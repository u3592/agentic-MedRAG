...
def create_digester_node(chain, retrieve_context):
    def node(state: GraphState) -> GraphState:
        ...
        for q in queries:
            snippets, _ = retrieve_context(q.text, k=4)
            try:
                result = chain.invoke({
                    "query": q.text,
        ...
        for i, hypothesis in enumerate(hypotheses):
            ...
            if state["step"] <= i:
                ...
                for premise in premises:
                    snippets, _ = retrieve_context(query=premise.text, k=4)
            ...
            prior_conclusions.append(hypothesis.conclusion)
...
def create_fixer_node(chain):
    def node(state: GraphState) -> GraphState:
        ...
        if state["retries"][step] >= state["max_retries"]:
            ...
            return state
        conclusions = [hypothesis.conclusion for hypothesis in state["hypotheses"][:step]]
        comment = state["comments"][step][-1] if state["comments"][step] else ""
        try:
            result = chain.invoke({
                "hypothesis": state["hypotheses"][step],
...
def create_evaluator_node(chain):
    def node(state: GraphState) -> GraphState:
        hypotheses = state["hypotheses"]
        content = "".join([
            "[{:d}] {:s}, Annotation: {:s}\n".format(i, hypothesis.json(), state["comments"][i][-1] if state["comments"][i] else "") for i, hypothesis in enumerate(hypotheses[:-1])
        ])
        try:
            result = chain.invoke({
                "question": state["question"],
...
class AgenticMedRAG:
    ...
    def answer(self, question: str, options: Dict[str, str]) -> str:
        ...
        if self.agents:
            ...
            # graph.add_edge("compiler", "evaluator")
